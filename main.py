...
def init_db():
    with sqlite3.connect(DATABASE) as conn:
        conn.execute('''CREATE TABLE IF NOT EXISTS videos (
                        ...
                        register_id INTEGER,
                        FOREIGN KEY (register_id) REFERENCES user(register_id))''')

        cursor.execute('''
        CREATE TABLE IF NOT EXISTS likes (
        reg_id INTEGER,
        video_id INTEGER,
        PRIMARY KEY (reg_id, reg_id),
        FOREIGN KEY (user_id) REFERENCES regster_user(reg_id),
...
def liked_video():
    conn = sqlite3.connect('video_likes.db')
    cursor = conn.cursor()
    ...
    if user_id:
        liked_videos = {}
        cursor.execute('SELECT video_id FROM likes WHERE user_id = ?', (user_id,))
    ...
    conn.close()
    return render_template('index.html', videos=videos, liked_videos=liked_videos)
    ...
    conn = sqlite3.connect('user.db')
    cursor = conn.cursor()
    cursor.execute('SELECT * FROM likes WHERE reg_id = ? AND video_id = ?', (reg_id, video_id))
    if cursor.fetchone():
        conn.close()
        return redirect(url_for('home'))  
    
    cursor.execute('''
    SELECT videos.video_id, videos.title, videos.likes, user.email 
    FROM videos 
    JOIN user ON videos.register_id = register_user.register_id
    ''')
    videos = cursor.fetchall()
    conn.close()
